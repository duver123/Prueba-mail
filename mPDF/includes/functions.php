...
if(!function_exists('strcode2utf')){ 
  function strcode2utf($str,$lo=true) {
	...
	/*$replace = code2utf('\\1', $lo);
	$str = preg_replace('/\&\#([0-9]+)\;/m', $replace, $str);
	$str = preg_replace('/\&\#x([0-9a-fA-F]+)\;/m', $replace, $str);*/
	$str = preg_replace_callback('/\&\#([0-9]+)\;/m', function($matched) {
		return code2utf($matched, $lo);
	}, $str);
	$str = preg_replace_callback('/\&\#x([0-9a-fA-F]+)\;/m', function($matched) {
		return code2utf($matched, $lo);
	}, $str);
	return $str;
  }
